...
function findBrand(identifier?: string | null): ToolBrand | null {
  ...
  return TOOL_BRANDS.find((brand) => brand.aliases.some((alias) => n.includes(alias))) ?? null;
}
...
// Shared icon renderers used by multiple tool keys
const recallIcon = (s: number) => (
  <svg
    width={s}
    height={s}
    viewBox="0 0 24 24"
...
const BOOP_ICONS: Record<string, (s: number) => ReactNode> = {
  ...
  write_memory: recallIcon,
  save_draft: draftIcon,
  send_draft: (s) => (
    <svg
      width={s}
      height={s}
  ...
  spawn_agent: (s) => (
    <svg
      ...
      strokeLinejoin="round"
    >
      <title>Spawn Agent</title>
      <path d="M12 8V4H8" />
      <path d="m12 4 4 4" />
      <rect width="16" height="12" x="4" y="8" rx="2" />
    </svg>
  ),
  create_automation: automationIcon,
  list_automations: automationIcon,
  toggle_automation: automationIcon,
  delete_automation: automationIcon,
  list_drafts: draftIcon,
  reject_draft: (s) => (
    <svg
      width={s}
      height={s}
