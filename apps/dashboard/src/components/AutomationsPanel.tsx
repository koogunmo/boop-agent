...
type Automation = NonNullable<FunctionReturnType<typeof api.automations.list>>[number];
type AutomationRun = NonNullable<FunctionReturnType<typeof api.automations.recentRuns>>[number];
...
const DEFAULT_STATUS_COLOR = { dot: "bg-sky-400 live-dot", text: "text-sky-400" } as const;
...
export function AutomationsPanel({ isDark }: { isDark: boolean }) {
  ...
  const enabledCount = list.filter((a: Automation) => a.enabled).length;
              ...
              <div className="flex items-center gap-2.5 mb-1.5">
                <button
                  ...
                  className={`relative inline-flex items-center w-9 h-5 rounded-full transition-colors shrink-0 ${
                    auto.enabled ? "bg-emerald-500" : isDark ? "bg-slate-700" : "bg-slate-300"
                  }`}
                >
                  <span
  ...
  return (
    <div className="flex flex-col h-full -m-5 fade-in">
      ...
      >
        ...
        <button
          type="button"
          onClick={() => setEnabled({ automationId: auto.automationId, enabled: !auto.enabled })}
          className={`relative inline-flex items-center w-9 h-5 rounded-full transition-colors shrink-0 ${
            auto.enabled ? "bg-emerald-500" : isDark ? "bg-slate-700" : "bg-slate-300"
          }`}
        >
        ...
        <span className={`text-xs ml-auto mono ${mutedText}`}>{formatSchedule(auto.schedule)}</span>
      ...
      >
        <div>
          <span
            className={`text-[10px] font-bold mono ${isDark ? "text-slate-600" : "text-slate-400"}`}
          >
            TASK{" "}
          </span>
      ...
      <div className="flex-1 overflow-y-auto debug-scroll">
        <div
          className={`px-5 py-2 border-b ${isDark ? "border-slate-800/50" : "border-slate-100"}`}
        >
          <span
        ...
        {runs === undefined ? (
          <div className="p-5 space-y-2">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className={`h-10 rounded shimmer ${isDark ? "bg-slate-900/30" : "bg-slate-100"}`}
              />
            ))}
          </div>
        ) : runs.length === 0 ? (
          ...
          <div className={`divide-y ${isDark ? "divide-slate-800/40" : "divide-slate-100"}`}>
            {runs.map((run: AutomationRun) => {
              const color = STATUS_COLOR[run.status] ?? DEFAULT_STATUS_COLOR;
              return (
                <div
                  key={run._id}
