...
export function DashboardPanel({ isDark }: { isDark: boolean }) {
  ...
  const filtered = useMemo(() => {
    ...
    const days = cutoff ? data.dailyBuckets.filter((d) => d.day >= cutoff) : data.dailyBuckets;
    ...
    <div className="h-full overflow-y-auto debug-scroll -m-5 p-5 space-y-5">
        ...
        >
          {RANGES.map((r) => (
            <button
              ...
              } ${r.id === "7d" ? "rounded-l-lg" : ""} ${r.id === "all" ? "rounded-r-lg" : ""}`}
            >
              {r.label}
            </button>
          ))}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
          ...
          info={{
            ...
            body: (
              <>
                ...
                <p className="mb-1.5">
                  If you're using your <strong>Claude Code subscription</strong> (the default),
                  you're paying a flat monthly rate — not these dollar amounts.
                </p>
                <p>
        ...
        <StatCard
          ...
          {...(Number(failPct) > 20 ? { color: isDark ? "text-rose-400" : "text-rose-600" } : {})}
          c={c}
        />
      </div>
...
function StatCard({
  ...
  return (
    <div className={`rounded-xl border p-3.5 ${c.card} relative`}>
      <div
        className={`text-[11px] font-medium uppercase tracking-wider ${c.label} flex items-center gap-1.5`}
      >
        <span>{label}</span>
        {info && (
          <div className="relative" ref={popRef}>
            ...
            {open && (
              ...
              >
                <div
                  className={`font-semibold mb-1 ${isDark ? "text-slate-100" : "text-slate-900"}`}
                >
                  {info.title}
                </div>
      ...
      <div className={`text-xl font-bold mono mt-1 ${color ?? c.value}`}>{value}</div>
...
function BarRow({
  ...
  return (
    <div className="flex items-center gap-2 text-xs">
      <span className={`w-24 truncate capitalize ${isDark ? "text-slate-400" : "text-slate-600"}`}>
        {label}
      </span>
      <div
...
function StackedAreaChart({
  ...
  const areaEntries: { key: string; path: string; color: string }[] = [];
  for (let k = keys.length - 1; k >= 0; k--) {
    const topPoints = stacked.map((d, i) => `${x(i)},${y(d.layers[k] ?? 0)}`).join(" L");
    const bottomLayer =
      k > 0
        ? stacked
            .map((d, i) => `${x(i)},${y(d.layers[k - 1] ?? 0)}`)
            .reverse()
            .join(" L")
        : stacked
            .map((_, i) => `${x(i)},${y(0)}`)
    ...
    areaEntries.push({
      key: keys[k] ?? `area-${k}`,
      path: `M${topPoints} L${bottomLayer} Z`,
      color: colors[k] ?? "",
    });
  }
  ...
  for (let i = 0; i < data.length; i += step) {
    const entry = data[i];
    if (entry) xLabels.push({ i, label: (entry.day as string).slice(5) });
  }
  const lastEntry = data[data.length - 1];
  if (lastEntry && xLabels[xLabels.length - 1]?.i !== data.length - 1) {
    xLabels.push({
      i: data.length - 1,
      label: (lastEntry.day as string).slice(5),
    });
  }
    ...
    <div ref={containerRef} className="relative">
      ...
      >
        ...
        {areaEntries.map((entry) => (
          <path key={entry.key} d={entry.path} fill={entry.color} opacity={0.35} />
        ))}

        {keys.map((keyName) => {
          const kIdx = keys.indexOf(keyName);
          const linePoints = stacked.map((d, i) => `${x(i)},${y(d.layers[kIdx] ?? 0)}`).join(" L");
          return (
            <path
              key={keyName}
              d={`M${linePoints}`}
      ...
      {hoverIdx !== null && hovered && (
        ...
        >
          <div className={`font-semibold mb-1.5 ${isDark ? "text-slate-300" : "text-slate-700"}`}>
            {hovered.day}
          </div>
          {keys.map((keyName) => {
            const kIdx = keys.indexOf(keyName);
            return (
              <div key={keyName} className="flex items-center gap-2 py-0.5">
                <span
                  className="w-2 h-2 rounded-sm shrink-0"
                  style={{ background: colors[kIdx] }}
                />
                <span className={isDark ? "text-slate-400" : "text-slate-500"}>{labels[kIdx]}</span>
                <span className="ml-auto mono font-medium pl-3">
                  {format(hovered.raw[kIdx] ?? 0)}
                </span>
              </div>
            );
          })}
      ...
      <div className="flex items-center gap-4 mt-2 ml-14">
        {labels.map((l) => {
          const lIdx = labels.indexOf(l);
          return (
            <div key={l} className="flex items-center gap-1.5 text-[11px]">
              <span className="w-2.5 h-2.5 rounded-sm" style={{ background: colors[lIdx] }} />
              <span className={isDark ? "text-slate-400" : "text-slate-600"}>{l}</span>
            </div>
          );
        })}
      </div>
