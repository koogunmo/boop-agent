...
export function MemoryPanel({ isDark }: { isDark: boolean }) {
  ...
  const records = useQuery(api.memoryRecords.list, {
    ...(tierFilter !== "all" ? { tier: tierFilter as "short" | "long" | "permanent" } : {}),
    lifecycle: "active",
    limit: 500,
  });

  const allRecords = records ?? [];
  const filtered = allRecords.filter((r: MemoryRecord) => {
    ...
    <div className="flex flex-col h-full -m-5">
      ...
      >
        ...
        {viewMode === "table" && (
          <>
            ...
            <span className={`text-xs mono ${isDark ? "text-slate-600" : "text-slate-400"}`}>
      ...
      {viewMode === "graph" && (
        <div className="flex-1 min-h-0">
          <MemoryGraphView records={allRecords} isDark={isDark} />
        </div>
      )}
              ...
              {filtered.map((r: MemoryRecord) => {
                ...
                return (
                  <button
                    type="button"
                    key={r.memoryId}
                    className={`px-5 py-3 cursor-pointer transition-colors text-left w-full ${
                      isDark ? "hover:bg-slate-900/40" : "hover:bg-slate-50"
                    }`}
                    onClick={() => setExpandedId(isExpanded ? null : r.memoryId)}
                  >
                      ...
                      <div className="mt-3 space-y-2 text-xs slide-down">
                        ...
                        >
                          <div>
                            ID:{" "}
                            <span
                              className={`mono ${isDark ? "text-slate-400" : "text-slate-600"}`}
                            >
                              {r.memoryId}
                            </span>
                          </div>
