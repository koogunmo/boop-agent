  ...
  return (
    ...
    >
      <div className="min-w-0 flex-1">
        ...
        <div
          className={`text-xs mt-1 leading-relaxed ${isDark ? "text-slate-400" : "text-slate-600"}`}
        >
          {description}
        </div>
        <div className={`text-[10px] mono mt-2 ${isDark ? "text-slate-600" : "text-slate-400"}`}>
          {debugLine}
        </div>
      </div>
...
function ToggleRow({ setting, isDark }: { setting: ToggleSetting; isDark: boolean }) {
...
function TimezoneRow({ setting, isDark }: { setting: TimezoneSetting; isDark: boolean }) {
  const value = useQuery(api.settings.get, { key: setting.key });
  const setSetting = useMutation(api.settings.set);
