...
const DEFAULT_STATUS = { dot: "bg-sky-400", label: "Running", color: "text-sky-400" } as const;
...
export function AgentsPanel({ isDark }: { isDark: boolean }) {
  ...
  if (selected) {
    return <AgentDetail agentId={selected} onBack={() => setSelected(null)} isDark={isDark} />;
  }
    ...
    <div className="flex flex-col h-full -m-5">
      ...
      <div className="flex-1 overflow-y-auto debug-scroll p-4 space-y-3">
        {agents === undefined ? (
          ...
          filtered.map((agent) => {
            const cfg = STATUS_CONFIG[agent.status] ?? DEFAULT_STATUS;
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5 mb-1.5">
                  ...
                  <span className={`flex items-center gap-2 text-xs ml-auto ${cfg.color}`}>
...
function AgentDetail({
  ...
  if (!agent) {
    return (
      <div className="p-5">
        <div className={`h-20 rounded-xl shimmer ${isDark ? "bg-slate-900/40" : "bg-slate-100"}`} />
      </div>
    );
  }
    ...
    <div className="flex flex-col h-full -m-5 fade-in">
      ...
      >
        ...
        <div className="ml-auto flex items-center gap-3 text-xs mono">
          {agent.costUsd > 0 && (
            <span className="text-emerald-500 font-semibold">${agent.costUsd.toFixed(4)}</span>
          )}
          {totalTokens > 0 && (
      ...
      <div className="shrink-0 p-4 pb-2">
        <div
          className={`rounded-xl border shadow-lg px-4 py-3 ${
            isDark ? "bg-slate-900/90 border-sky-800/40" : "bg-white border-sky-200"
          }`}
        >
          <button
            type="button"
        ...
        {logs === undefined ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className={`h-8 rounded shimmer ${isDark ? "bg-slate-900/30" : "bg-slate-100"}`}
              />
            ))}
          </div>
        ) : logs.length === 0 ? (
          ...
          <div className="space-y-0">
            {logs.map((log, i) => (
              <TimelineRow key={log._id} log={log} isLast={i === logs.length - 1} isDark={isDark} />
            ))}
          </div>
        )}
      </div>
      ...
      {agent.status === "completed" && agent.result && (
        <div className="sticky bottom-0 p-4 pt-2">
          <div
            className={`rounded-xl border shadow-lg px-4 py-3 ${
              isDark ? "bg-slate-900/90 border-emerald-800/40" : "bg-white border-emerald-200"
            }`}
          >
            <button
              type="button"
      ...
      {agent.status === "failed" && agent.error && (
        <div className="sticky bottom-0 p-4 pt-2">
          <div
            className={`rounded-xl border shadow-lg px-4 py-3 ${
              isDark ? "bg-slate-900/90 border-rose-800/40" : "bg-white border-rose-200"
            }`}
          >
            <span className="text-[10px] font-bold mono tracking-wider text-rose-500">ERROR</span>
            <p
    ...
    <div className="flex gap-3">
      <div className="flex flex-col items-center shrink-0 w-5">
        <div className="mt-1.5">
          {isToolUse ? (
            <IntegrationLogo raw={log.toolName ?? null} size={20} />
          ) : (
            <span
        ...
        {!isLast && (
          <div className={`flex-1 w-px mt-1 ${isDark ? "bg-slate-800" : "bg-slate-200"}`} />
        )}
      </div>
