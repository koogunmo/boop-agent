...
export function EventsPanel({ isDark }: { isDark: boolean }) {
  ...
  const card = isDark ? "bg-slate-900/40 border-slate-800" : "bg-white border-slate-200";
  const row = isDark ? "bg-slate-900/50 border-slate-800" : "bg-slate-50 border-slate-200";
  const muted = isDark ? "text-slate-500" : "text-slate-400";

  return (
    <div className={`rounded-lg border p-4 ${card}`}>
      <h2 className={`text-xs uppercase tracking-wider mb-3 ${muted}`}>Recent events</h2>
      {!events ? (
