    ...
    <div className="flex flex-col h-full -m-5">
      ...
      <div className="flex-1 overflow-y-auto debug-scroll p-4 space-y-3">
        {runs === undefined ? (
          ...
          >
            No consolidation runs yet. The loop runs daily, or hit "Run now" to trigger one.
            <p className={`text-xs mt-2 ${muted}`}>
              Consolidation reviews your memories for duplicates and contradictions, merges or
              prunes via a proposer → judge pipeline.
            </p>
          </div>
        ) : (
          list.map((run) => {
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2.5 mb-1.5">
                  ...
                  <span className={`text-[10px] mono ${muted}`}>trigger: {run.trigger}</span>
                ...
                <div className="flex items-center gap-4 ml-5 text-[11px] mono">
                  ...
                  />
                  {run.notes && <span className={`${muted} truncate`}>{run.notes}</span>}
                </div>
              </button>
            );
          })
  ...
  return (
    <div className="flex flex-col h-full -m-5 fade-in">
      ...
      >
        ...
        <span className={`relative flex h-2.5 w-2.5 shrink-0 ${statusCfg.color}`}>
          {run.status === "running" && (
            <span
              className={`absolute inline-flex h-full w-full rounded-full ${statusCfg.dot} pulse-ring`}
            />
        ...
        <span className={`text-xs mono ml-auto ${muted}`}>trigger: {run.trigger}</span>
      </div>
      ...
      >
        <SummaryStat
          ...
          isDark={isDark}
        />
        <SummaryStat label="merged" value={run.mergedCount} color="text-sky-400" isDark={isDark} />
        <SummaryStat label="pruned" value={run.prunedCount} color="text-rose-400" isDark={isDark} />
        <SummaryStat
          label="duration"
          value={
            run.completedAt && run.startedAt
                ...
                return (
                  <div key={`${p.phase}-${p.ts}`} className="flex gap-3 slide-down">
                    <div className="flex flex-col items-center shrink-0 w-5">
                      ...
                      {!isLast && (
                        <div
                          className={`flex-1 w-px mt-1 ${isDark ? "bg-slate-800" : "bg-slate-200"}`}
                        />
                      )}
                    </div>
                    <div className="flex-1 min-w-0 pb-4">
                      <div className="flex items-center gap-2 mb-0.5">
                        <span className={`text-[10px] font-bold mono tracking-wider ${cfg.color}`}>
                          {cfg.label}
                        </span>
                      ...
                      <div
                        className={`text-xs ${isDark ? "text-slate-400" : "text-slate-600"} mono`}
                      >
