...
composio.get("/toolkits", async (c) => {
  const client = createComposioClient(c.env);
  if (!client) return c.json({ enabled: false, toolkits: [] });
  ...
  try {
    ...
    return c.json({ enabled: true, toolkits });
  } catch (err) {
...
composio.get("/toolkits/:slug/tools", async (c) => {
  const client = createComposioClient(c.env);
  if (!client) return c.json({ tools: [] });
...
composio.post("/toolkits/:slug/authorize", async (c) => {
  const client = createComposioClient(c.env);
...
composio.post("/toolkits/:slug/disconnect", async (c) => {
  const client = createComposioClient(c.env);
  if (!client) return c.json({ error: "COMPOSIO_API_KEY not set" }, 500);
