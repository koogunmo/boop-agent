...
describe("GET /api/composio/toolkits/:slug/tools", () => {
  ...
  it("returns empty tools when no client", async () => {
    disableComposio();
    const res = await req("/api/composio/toolkits/gmail/tools");
    expect(res.status).toBe(200);
...
describe("POST /api/composio/toolkits/:slug/authorize", () => {
  afterEach(() => vi.restoreAllMocks());
...
describe("POST /api/composio/connections/:id/rename", () => {
  ...
  it("returns 400 when alias missing", async () => {
    ...
    expect(res.status).toBe(400);
    const body = (await res.json()) as { error: string };
    expect(body.error).toBe("alias required in body");
  });
});

describe("POST /api/composio/refresh", () => {
  afterEach(() => vi.restoreAllMocks());
